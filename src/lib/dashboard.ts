...
export async function getDashboardData() {
  ...
  const activeSessions = (db.prepare("SELECT COUNT(*) AS count FROM sessions WHERE expires_at > ?").get(now.toISOString()) as { count: number }).count;
  const newToday = (db.prepare("SELECT COUNT(*) AS count FROM users WHERE created_at >= ?").get(today) as { count: number }).count;
  const recentRows = db.prepare("SELECT id, name, username, email, created_at FROM users ORDER BY created_at DESC LIMIT 5").all() as Array<{ id: string; name: string; username: string; email: string; created_at: string }>;
