...
function replaceTagsInParagraph(paragraphXml: string, data: Record<string, string>) {
...
function visibleTags(documentXml: string) {
  ...
  for (const paragraph of documentXml.matchAll(paragraphPattern)) {
    ...
    for (const tag of text.matchAll(/<([^<>]{1,80})>/g)) tags.add(tag[1]);
  }
  return [...tags];
}
...
export async function generateDpcpDocument(data: Record<string, string>) {
  ...
  const renderedXml = documentFile.asText().replace(
    paragraphPattern,
    (paragraph) => replaceTagsInParagraph(paragraph, data),
  );
  const leftovers = visibleTags(renderedXml);
