...
export async function savePakDocument(userId: string, employee: { name: string; status: string }, input: PakInput, document: Buffer) {
  ...
  try {
    db.prepare(`INSERT INTO pak_documents (id, user_id, employee_nip, employee_name, employee_status, nomor, tanggal, period, total, input_json, file_name, storage_name, file_size, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, input.nip, employee.name, employee.status, input.nomor, input.tanggal, `${periodLabel(input.period)} ${input.period.year}`, calculatePak(input).total, JSON.stringify({ ...input, employeeName: employee.name, employeeStatus: employee.status }), fileName, storageName, document.length, new Date().toISOString());
  } catch (error) { await rm(filePath, { force: true }); throw error; }
  return { id, fileName };
}
export async function getPakHistory(): Promise<PakDocumentHistory[]> {
  await requireUser();
  return db.prepare(`SELECT d.id, d.employee_nip AS employeeNip, d.employee_name AS employeeName, d.employee_status AS employeeStatus, d.nomor, d.tanggal, d.period, d.total, d.file_name AS fileName, d.created_at AS createdAt, u.name AS createdBy FROM pak_documents d JOIN users u ON u.id = d.user_id ORDER BY d.created_at DESC`).all() as PakDocumentHistory[];
}
export function getPakDocumentForDownload(id: string) {
  const row = db.prepare("SELECT file_name, storage_name FROM pak_documents WHERE id = ?").get(id) as { file_name: string; storage_name: string } | undefined;
